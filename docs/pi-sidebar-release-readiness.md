...
Tracking note for the fork-local `pi-sidebar` delivery branch and the webview-native Pi size-fix tail. This file records the current repository state as of 2026-07-07 after the branch was refreshed through application-code commit `2694095a3`. Older sidecar-era notes are historical only.
...
- CI run `28910393808` completed successfully for PR head `6c8e60ddb74f83d75b7bd9b3c79acf598b9ff612`: `frontend`, `rust`, `rust-test (windows-latest)`, `rust-test (macos-latest)`, `coverage`, and `e2e (linux)` all completed with `SUCCESS`.
- CI independently ran on the PR, including the Linux e2e job and the Pi approval spec. Re-check the current pushed head after any later docs-only tail commit.
...
| Done | Confirm CI/e2e green. | CI run `28910393808` completed successfully for PR head `6c8e60d`; `frontend`, `rust`, `rust-test (windows-latest)`, `rust-test (macos-latest)`, `coverage`, and `e2e (linux)` are all `SUCCESS`. The latest e2e fix in `2694095a3` uses DOM clicks for transcript approval controls that WebDriver saw but could not click directly under WebKit content visibility. Re-check the current pushed head after any later docs-only tail commit. | None for the inspected PR CI/e2e run. |
...
| Done | Pass pnpm and Rust verification gates. | Local checks after the latest tail: `node scripts/check-pi-approval-boundary.mjs`, `pnpm check:pi-boundary`, and `git diff --check` passed. Earlier same-head-family local checks: `pnpm test` passed 199 files and 1179 tests, targeted proxy tests passed, `pnpm check-types` passed, `pnpm format:check` passed, `pnpm lint` exited 0 with baseline warnings, `pnpm check:updater-key-rotation` passed, `pnpm check:bundle-size` passed at `1776.7 KB`, `pnpm build` passed with existing Rolldown/Hugeicons `INVALID_ANNOTATION` warnings, and `pnpm tauri build --bundles app --no-sign --ci` produced an `11M` app plus `6.6M` updater tarball. CI run `28910393808` passed frontend, Rust default/workflow/openclicky matrix coverage, coverage, and Linux e2e. | None for automated gates. |
...
Rust checks are also covered by PR CI run `28910393808`. Prior local Rust checks remain relevant because the latest changes touched only frontend TypeScript proxy handling, tests, e2e selectors, the Pi transcript test, and docs:
...
3. Promote Pi composer to default and collapse/rename residual runtime layers only after manual smoke and updater verification are green.
