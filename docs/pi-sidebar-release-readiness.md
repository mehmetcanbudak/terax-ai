...
Tracking note for the fork-local `pi-sidebar` delivery branch and the webview-native Pi size-fix tail. This file records the current repository state as of 2026-07-07 after the local branch was refreshed through application-code commit `6d1bf0d71`. Older sidecar-era notes are historical only.
...
- Local merge audit: `git merge-tree --write-tree HEAD origin/main` exits 0 and produced tree `b6b1744ec9fd0e0fde922ceb47dd02d857830db0`.
...
- Most recently inspected remote check rollup for commit `e6563529d`: `frontend`, `rust`, `rust-test (windows-latest)`, `rust-test (macos-latest)`, and `coverage` were successful; `e2e (linux)` was still in progress. Current local commits after that run need the final PR CI/e2e confirmation after push. Final CI/e2e confirmation is deferred until all non-CI work is done.
...
| --- | --- | --- | --- |
| Done | Commit and push `pi-sidebar`; open PR. | Fork PR #1 is open at <https://github.com/mehmetcanbudak/terax-ai/pull/1>; the branch includes application-code head `6d1bf0d71` with release-readiness docs on top. | None for fork-local PR creation and push after the final docs refresh is pushed. |
| Done | Resolve merge conflicts against current `origin/main`. | `git merge-tree --write-tree HEAD origin/main` exits 0 against `origin/main` `78a0b3dd79554ad4af89e61d97004f3475cd9953` and produced tree `b6b1744ec9fd0e0fde922ceb47dd02d857830db0`; PR reports `mergeable=MERGEABLE`. | None locally. |
| Deferred | Confirm CI/e2e green. | Latest inspected remote PR check rollup had all non-e2e jobs successful and `e2e (linux)` in progress for `e6563529d`. The prior e2e failure was fixed by commit `e6563529d`, which added stable Pi approval controls and updated the e2e selector path. Current local tail commits need final PR CI/e2e confirmation after push. | Final CI/e2e confirmation is deferred until all non-CI work is done. |
...
| Done | Keep default app about 11 MB. | Latest local signed-artifact-size carry-forward from the post-merge package check reported `10M` for `Terax.app` and `7.0M` for `Terax.app.tar.gz`. The latest `pnpm check:bundle-size` on `e6563529d` reported `1776.6 KB` gzipped JS against the `2050.8 KB` budget, and the local tail commit only changed proxy body handling plus tests/docs. | Re-run on the final signed release artifact. |
...
| Done locally | Pass pnpm and Rust verification gates. | Latest local checks after `6d1bf0d71`: `pnpm test` passed 199 files and 1179 tests, `pnpm check-types` passed, targeted proxy tests passed, `pnpm check:pi-boundary` passed, and `git merge-tree --write-tree HEAD origin/main` passed. Carried-forward checks after `e6563529d`: `pnpm format:check` passed, `pnpm lint` exited 0 with baseline warnings, `pnpm check:updater-key-rotation` passed, `pnpm check:bundle-size` passed, and `pnpm build` passed with existing Rolldown/Hugeicons `INVALID_ANNOTATION` warnings. Prior post-merge Rust validation passed `cargo fmt -- --check`, `cargo check --locked`, `cargo check --all-targets --locked`, `cargo clippy --locked --all-targets -- -D warnings`, `cargo test --locked`, `cargo check --locked --features workflow`, `cargo check --locked --features openclicky`, `cargo test --locked --features workflow`, and `cargo test --locked --features openclicky`. | CI must independently run on the PR. |
...
git merge-tree --write-tree HEAD origin/main # exits 0, tree b6b1744ec9fd0e0fde922ceb47dd02d857830db0
```

Rust checks previously completed on this branch and remain relevant because the latest changes touched only frontend TypeScript proxy handling, tests, e2e selectors, the Pi transcript test, and docs:
